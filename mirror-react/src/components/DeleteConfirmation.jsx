...
export default function DeleteConfirmation({ onClose, onProdutoRemovido }) {
      ...
      <div className="w-[350px] h-115 bg-white rounded-2xl shadow-md flex flex-col items-center justify-center font-['Montserrat']">
        <div className="text-center p-6">
          ...
          <p className="text-sm text-gray-500 mb-4">
            {error || "Aguarde enquanto carregamos os dados do produto selecionado"}
          </p>
          <button
            onClick={onClose}
  ...
  // Usar o nome do produto como texto de confirmação
  const confirmationText = produtoCompleto.nome
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8080/api/itens-pedidos/${produtoCompleto.id}`, {
        method: "DELETE",
        headers: {
      ...
      alert(`Produto "${produtoCompleto.nome}" excluído com sucesso!`)
        ...
        <div className="px-4 py-3 bg-red-50 border border-red-200 rounded-md mb-4">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 border border-red-300 rounded-md overflow-hidden flex items-center justify-center bg-white">
              <img
                src={produtoCompleto.imagem || "/placeholder.svg"}
                alt={produtoCompleto.nome}
                className="max-w-full max-h-full object-contain"
              />
            </div>
            <div className="flex-1">
              <p className="text-sm font-semibold text-red-800">{produtoCompleto.nome}</p>
              <p className="text-xs text-red-600">
                ID: {produtoCompleto.id} • {produtoCompleto.categoria}
              </p>
              <p className="text-xs text-red-600">Preço: R$ {produtoCompleto.preco}</p>
            </div>
          </div>
        </div>

        <p className="text-gray-600 !mb-4 leading-relaxed text-center">
          Você está prestes a excluir <strong>"{produtoCompleto.nome}"</strong> do cardápio. Esta ação não pode ser
          desfeita.
        </p>
        <p className="text-gray-600 !mb-4 text-center">Para confirmar, digite o nome exato do produto abaixo:</p>
        ...
        <div className="flex justify-center gap-3 !mb-8">
          ...
          <button
            className={`!px-4 !py-2.5 border-none ${
              isConfirmEnabled && !loading ? "bg-[#e30613] hover:bg-[#c00]" : "bg-red-200 cursor-not-allowed"
